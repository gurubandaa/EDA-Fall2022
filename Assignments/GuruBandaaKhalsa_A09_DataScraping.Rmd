...
electricity_profiles <- data.frame(
    Water_System = water.system.name,
    PWSID = pwsid,
    Ownership = ownership,
    Max_Day_Use = max.withdrawals.mgd
    ) %>%
    mutate(Month = Month) %>%
    mutate(Year = Year) %>%
...
#   for 2021.
ggplot(electricity_profiles, aes(x = Date, y = max.withdrawals.mgd)) + 
  geom_line(aes(group = 1)) + 
  geom_smooth(method = "loess", se = FALSE) +
  labs(title = paste("2021 Monthly Maximum Daily Withdrawals"),
...
scrape.it <- function(Year){
  the_url <- ifelse(Year==2021,
                    'https://www.ncwater.org/WUDC/app/LWSP/report.php?pwsid=03-32-010',
                    paste0('https://www.ncwater.org/WUDC/app/LWSP/report.php?pwsid=03-32-010&year=',Year))
  
  webpage2 <- read_html(paste0("https://www.ncwater.org/WUDC/app/LWSP/report.php?pwsid=03-32-010&year="), Year)
  }

scrape.it2 <- function(pwsid){
  the_url <- ifelse(pwsid==03-32-010,
                    'https://www.ncwater.org/WUDC/app/LWSP/report.php?pwsid=03-32-010&year=2021',
                    paste0('https://www.ncwater.org/WUDC/app/LWSP/report.php?pwsid=', pwsid, '&year=2021'))
  
  webpage3 <- read_html(paste0("https://www.ncwater.org/WUDC/app/LWSP/report.php?pwsid="), pwsid)
  }

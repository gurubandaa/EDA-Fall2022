...
EPAair.dropna <- drop_na(EPAair)

TPAIC <- lm(data = EPAair.dropna, PM2.5 ~ Ozone)

step(TPAIC)
TPmodel <- lm(data = EPAair.dropna, PM2.5 ~ Ozone)
summary(TPmodel)
...
# If you remove Year, you are going to improve your AIC.  Year should be removed from the series (AIC without Year is one unit less than with Year).  Continue removing one at a time.

OzonebyBEST.regression <- lm(data = EPAair.subset, PM2.5 ~ Ozone + Month + meanLng + meanLat)
summary(OzonebyBEST.regression)
step(OzonebyBEST.regression)
...
assign.group <- function(site)
  return(Litter.Totals.groups[as.character(site), 2])
...
Litter.Totals$plotID <-
  reoreder(Litter.Totals$plotID, Litter.Totals$dryMass, FUN = mean)

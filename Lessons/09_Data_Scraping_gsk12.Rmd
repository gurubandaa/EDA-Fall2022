...
# Some scratch work from A09
Years <- c(2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021)

scrape.it <- function(pwsid, Years){
  webpage <- read_html(paste0('https://www.ncwater.org/WUDC/app/LWSP/report.php?pwsid=', pwsid, '&year=', Years))
...
pwsids <- webpage %>% html_nodes("td tr:nth-child(1) td:nth-child(5)") %>% html_text()
pwsids <- sample(pswids, 1)
Years <- c(2010:2021)

the_df2 <- cross2(pwsids, Years) %>%
  map(lift(scrape.it)) %>%
  bind_rows()


scrape.it <- function(pwsid, Year){
  webpage <- read_html(paste0('https://www.ncwater.org/WUDC/app/LWSP/report.php?pwsid=', pwsids, '&year=', Years))
...
pwsid3 = '01-11-010'
asheville_decade <- lapply(X = the_years,
                           FUN = scrape.it2,
                           pwsid = pwsid3)
asheville_decade_bind <- bind_rows(asheville_decade)

ggplot(asheville_decade_bind) +
  geom_line(aes(x = Month, y = max.withdrawals.mgd, color = Year))


#Subset the facilities
withdrawals_subset <-  sample(max.withdrawals.mgd, 10)


scrape.it2 <- function(pwsid2, Year){

  webpage <- read_html(paste0('https://www.ncwater.org/WUDC/app/LWSP/report.php?pwsid=', pwsid2, '&year=', Year))

  water.system.name <- webpage %>% html_nodes("div+ table tr:nth-child(1) td:nth-child(2)") %>% html_text()
  pwsid2 <- webpage %>% html_nodes("td tr:nth-child(1) td:nth-child(5)") %>% html_text()
  ownership <- webpage %>% html_nodes("div+ table tr:nth-child(2) td:nth-child(4)") %>% html_text()
  max.withdrawals.mgd <- webpage %>% html_nodes("th~ td+ td") %>% html_text()

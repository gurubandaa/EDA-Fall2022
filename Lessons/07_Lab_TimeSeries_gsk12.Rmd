...
rainfall.plot <- ggplot(BRA_complete, aes(x = Date, y = Rainfall)) +
  geom_line() +
  geom_smooth()
print(rainfall.plot)

temperature.plot <- ggplot(BRA_complete, aes(x = Date, y = Temperature_C)) +
  geom_line() +
...
BRA_rain_ts <- ts(BRA_complete$Rainfall, frequency = 12, start = c(f_year, f_month))

BRA_temp_ts <- ts(BRA_complete$Temperature_C, frequency = 12, start = c(f_year, f_month))
...
rain_decomp <- stl(BRA_rain_ts,s.window = "periodic")
plot(rain_decomp)

temp_decomp <- stl(BRA_temp_ts,s.window = "periodic")
plot(temp_decomp)
...
temp_Components <- as.data.frame(rain_decomp$time.series[,1:3])

rain_Components <- mutate(rain_Components as.data.frame(rain_decomp$time.series[,1:3])

temp_Components <- mutate(temp_Components as.data.frame(rain_decomp$time.series[,1:3])
